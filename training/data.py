...
class PackedDataCollator:
    ...
    def _pack_sequences(self, sequences: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # Pack sequences
        input_ids = torch.cat(input_sequences, dim=0) if input_sequences else torch.tensor([], dtype=torch.long)
        targets = torch.cat(target_sequences, dim=0) if target_sequences else torch.tensor([], dtype=torch.long)
