...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download a specific file from a Hugging Face repo"
    )
    parser.add_argument("repo_id", type=str, help="HF repo id, e.g. cartesia-ai/hnet_2stage_XL")
    parser.add_argument(
        "filename", type=str, help="Filename inside the repo, e.g. pytorch_model.pt"
    )
    parser.add_argument("--outdir", type=str, help="Destination directory")
    parser.add_argument("--cache-dir", type=str, help="HF cache directory")
    ...
    try:
        ...
        print("huggingface_hub is required: uv pip install huggingface_hub")
        sys.exit(1)
    ...
    snapshot_dir = snapshot_download(
        repo_id=args.repo_id, cache_dir=args.cache_dir, allow_patterns=[args.filename]
    )
